...
bool SW_DropPod::Activate(SuperClass* pThis, CellStruct Coords, bool IsPlayer)
{
	...
	for(int i = 3 * count; i; --i) {
		...
		CellStruct tmpCell = MapClass::Instance.NearByLocation(cell, SpeedType::Wheel, -1,
		...
		for(int j = 0; j < 8; ++j) {
			...
			CellClass* pNeighbour = pCell->GetNeighbourCell(static_cast<FacingType>(dir));
